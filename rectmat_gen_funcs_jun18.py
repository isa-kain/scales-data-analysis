...
def remove_spot_dups(spot_tracks,lams_u,maxdist=13,lmin=2.9,lmax=4.15,chx=1,chy=1,medres=False):
    """
    ...
    for i in range(len(spots_d)):
        ...
        if medres==True:
            xi = x0
            yi,ymin,ymax=get_spot_yrange_medres(lam,y0=y0,lam0=lam0,chy=1,lmin=1.95,lmax=2.45,length=1822)
            ...
            for j in range(len(spots_d)):
                if i != j:
                    ...
                    if j not in rem:
                        ...
                        if medres==True:
                            xj = x0
                            yj,ymin,ymax=get_spot_yrange_medres(lam,y0=y0,lam0=lam0,chy=1,lmin=1.95,lmax=2.45,length=1822)
...
def get_spot_yrange_medres(lam,y0=180,lam0=2.0248,chy=1,lmin=1.95,lmax=2.45,length=1822):
    """
...
def remove_silos(avgs,spot_tracks,medres=True,show_plots=False):
    """
...
def get_lensarr_xy(avgs,show_plots=False,medres=True):
    """
    ...
    print('<< DEBUG >> shape avgs[0][0]', np.shape(avgs[0]), np.shape(avgs[0][0]), avgs[0])

    posns_pix[1000,1000] = avgs[0] # GUESSING AT FIXES
    ...
    while len(done_searching_around) < len(avgs):
        ...
        for search_lens in to_search_around:
            ...
            print("search lens pos:",avgs[search_lens:search_lens+1])
...
def get_medres_lensarr_xy(avgs,show_plots=True):
    inds0 = np.array(range(len(avgs)),dtype='int')
    row1 = avgs[np.where(avgs[:,1] < np.min(avgs[:,1]) + 20)]
    row1inds = inds0[np.where(avgs[:,1] < np.min(avgs[:,1]) + 20)]
    rem = np.delete(avgs,np.where(avgs[:,1] < np.min(avgs[:,1]) + 20),axis=0)
    indsrem = np.delete(inds0,np.where(avgs[:,1] < np.min(avgs[:,1]) + 20),axis=0)
    row2 = rem[np.where(rem[:,1] < np.min(rem[:,1])+20)]
    row2inds = indsrem[np.where(rem[:,1] < np.min(rem[:,1])+20)]
    row3 = np.delete(rem,np.where(rem[:,1] < np.min(rem[:,1])+20),axis=0)
    row3inds = np.delete(indsrem,np.where(rem[:,1] < np.min(rem[:,1])+20),axis=0)
    ...
    for i in range(len(scol_map)):
        ...
        for j in range(len(cols)):
            col = cols[j]
            print(row,col)
            #print(posns_idx[row,col*17:(col+1)*17])
            #print(posns_idx[row,col*17:(col+1)*17].shape)
            #print(posns_pix[row,col*17:(col+1)*17,2:4])
            ###this assumes that left to right we are moving bottom to top!!!!
...
def gen_rectmat_inds(calims,posarr,cut=0.25,method='sum'):
        ...
        for lensx in range(posarr.shape[2]):
            for lensy in range(posarr.shape[1]):
                ...
                if np.isnan(xc)==False:
                    ...
                    vals = crop_sparse_vals(calims[ll],xs,xe,ys,ye,cut=cut,method=method)
...
def crop_sparse_vals(image,xs,xe,ys,ye,cut=0.5,method='sum'):
    """
    ...
    #"""
    bkgs = np.array([list(image[ys-3:ys,xs:xe].flatten())+
                     list(image[ye:ye+3:,xs:xe].flatten())+
                     list(image[ys:ye,xs-3:xs].flatten())+
                     list(image[ys:ye,xe:xe+3].flatten())])
    bkg = np.mean(bkgs)
    cropped = cropped-bkg
    #"""
    ...
    #cropped = cropped-np.min(cropped)
...
def gen_QL_rectmat(calims,posarr,cut=0.25,method='sum'):
    """
    Function to generate rectmat from cube of cal unit images.
    """

    matrowinds,matcolinds,matvals = gen_rectmat_inds(calims,posarr,cut=cut,method=method)
    rmat = sparse.csr_matrix((matvals,(matrowinds,matcolinds)),shape=(np.prod(posarr.shape[:3]),np.prod(calims[0].shape)))
    return rmat



def gen_c2_rectmat_inds(calims,posarr,cut=0.25):
...
def gen_C2_rectmat(calims,posarr,cut=0.25):
    """
    Function to generate rectmat from cube of cal unit images.
    """

    print('doing c2 rectmat lowres')
    matrowinds,matcolinds,matvals = gen_c2_rectmat_inds(calims,posarr,cut=cut)
...
def find_all_spots(ims_cal, lams_u, min_distance=15, thresh=50, plot_im=False, sigma=0.8,medres=False):
    spots = {}
    ...
    for ii in range(len(ims_cal)):
        ...
        if medres==True:
            ycent,ymin,ymax=get_spot_yrange_medres(lams_u[ii])
            dy = (ymax-ymin)*0.5
            #print(ymin,ymax,ycent,dy,coords_yx[:,0])
            #plt.scatter(coords_yx[:,1],coords_yx[:,0])
            #plt.show()
            #coords_yx = coords_yx[np.where(np.abs(coords_yx[:,0]-ycent) < dy)]
            im_cal = im_cal[int(ymin):int(ymax)]
        data_smooth = gaussian_filter(im_cal, sigma=sigma)
        #data_smooth = median_filter(im_cal,size=7)
...
def find_avg_spotpos(spot_tracks_u,lmin,lmax,chx=1,chy=1,medres=False,show_plots=False):
    avgs = []
...
def make_posarr(ims_cal,final_posns,spot_tracks_u,medres=False,show_plots=False,cropsize=10):

    ###let's oversize the IFS datacube to 115 x 115
    #posarr = np.zeros([len(ims_cal),final_posns.shape[0],final_posns.shape[1],7])
